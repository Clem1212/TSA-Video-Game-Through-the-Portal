...
scene('title', () => {
  ...
  // Add stroke for "Witch Boss Fight" text
  add([
  ...
  add([
    ...
    color(1, 0, 0) // Main text color (red)
  ]);
...
scene('rules', () => {
  ...
  add([
    text('Rules:', 24),
    origin('center'),
    pos(width() / 2 + 2, height() / 4.5 + 2), // Offset for stroke effect
    color(0, 0, 0), // Stroke color (black)
  ]);
  add([
    text('Rules:', 24),
  ...
  add([
    ...
    origin('center'),
    pos(width() / 2 + 1, height() / 3 + 1), // Offset for stroke effect
    color(0, 0, 0), // Stroke color (black)
  ]);
  add([
    text('1. Use arrow keys and WASD to move', 14),
  ...
  // Add stroke for "Press Space to continue"
  add([
    text('Press Space to continue', 10),
    origin('center'),
    pos(width() / 2 + 1, height() / 1.2 + 1), // Offset for stroke effect
    color(0, 0, 0), // Stroke color (black)
  ]);
  add([
    text('Press Space to continue', 10),
  ...
  // Transition to the next rules scene
  keyPress('space', () => {
    go('rules2', { level: 0, score: 0 });
  });
});

// Rules Scene (Second Rule Set)
scene('rules2', () => {
  layers(['bg2', 'obj'], 'obj'); // 'obj' is the default layer
  ...
  // Add stroke for "Press Space to Play"
  add([
...
//not moving add 'varname' like 'virus2' if remvove it moves
 ...
 player.collides("Portal", (Portal) => {
  destroy(Portal);
  {
    window.location.href = "win.html"; // Navigate to win.html
}      });

player2.collides("Portal", (Portal) => {
  destroy(Portal);
  {
    window.location.href = "win.html"; // Navigate to win.html
}
