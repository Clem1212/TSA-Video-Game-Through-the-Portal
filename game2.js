...
scene('title', () => {
  ...
  // Add "Golden goose" text with stroke
  add([
      text('Golden goose', 32),
      origin('center'),
      pos(width() / 2 + 2, height() / 3 + 2), // Offset for stroke effect
      color(0, 0, 0), // Stroke color (black)
      scale(0.8), // Scale for stroke
  ]);
  add([
      text('Golden goose', 32),
      origin('center'),
      pos(width() / 2, height() / 3), // Main text position
      color(0, 1, 0), // Main text color (green)
      scale(0.8),
  ]);

  // Add "Press SPACE to view the rules" text
  add([
      text('Press SPACE to view the rules', 16),
      origin('center'),
      pos(width() / 2 + 1, height() / 2 + 1), // Slight offset for stroke
      color(0, 0, 0), // Stroke color (black)
  ]);
  add([
      text('Press SPACE to view the rules', 16),
      ...
      color(1, 1, 0), // Main text color (yellow)
  ]);
...
scene('rules', () => {
  ...
  // Add "Rules:" text with stroke
  add([
      text('Rules:', 24),
      origin('center'),
      pos(width() / 2 + 2, height() / 3.5 + 2), // Offset for stroke effect
      color(0, 0, 0), // Stroke color (black)
  ]);
  add([
      text('Rules:', 24),
  ...
  add([
    ...
    origin('center'),
    pos(width() / 2 + 1, height() / 1.1 + 1), // Offset for stroke effect
    color(1, 1, 1), // Main text color (black)
  ]);
  add([
    text('& activate Power Up.', 16),
